  ...
  Alpine.data("swim", () => ({
    ...
    calculateSwimTimesByPace() {
      ...
      this.swimTimesCommonDistances.forEach((swimTime) => {
        swimTime.time = calculateTime(
          seconds,
          swimTime.distance,
          swimTime.distanceUnit,
          100
        );
      });
      ...
      this.customSwimTime = calculateTime(
        seconds,
        this.customSwimDistanceInMeters,
        "m",
        100
      );
    },
    ...
    calculateSwimTimesByDistanceAndTime() {
      ...
      const seconds = minutesInput * 60 + secondsInput;
      const paceRatio = 100;
      const pacePer100InSeconds =
        (paceRatio / parseInt(this.swimDistanceInputInMeters)) * seconds;
    ...
    Alpine.data("bike", () => ({
      ...
      calculateBikeTimesByPace() {
        ...
        this.bikeTriathlonDistances.forEach((bikeDistance) => {
          bikeDistance.time = calculateTimeSegmentsFromSeconds(
            (bikeDistance.distance / pace) * 3600
          );
        });
        this.customBikeTime = calculateTimeSegmentsFromSeconds(
          (parseFloat(this.customBikeDistanceInKm) / pace) * 3600
        );
      },
      ...
      calculateBikeTimesByDistanceAndTime() {
        ...
        const bikeDistanceInputInKmParsed = parseFloat(
          this.bikeDistanceInputInKm
        );
        this.bikeTriathlonDistances.forEach((bikeDistance) => {
          bikeDistance.time = calculateTimeSegmentsFromSeconds(
            (bikeDistance.distance / bikeDistanceInputInKmParsed) * seconds
          );
        });
        this.customBikeTime = calculateTimeSegmentsFromSeconds(
          (parseFloat(this.customBikeDistanceInKm) /
            bikeDistanceInputInKmParsed) *
            seconds
        );
      },
    })),
    Alpine.data("run", () => ({
      ...
      calculateRunTimesByPace() {
        ...
        this.runTimesShortTrack.forEach((runTime) => {
          runTime.time = calculateTime(
            seconds,
            runTime.distance,
            runTime.distanceUnit,
            ratio
          );
        });
        ...
        this.customRunTime = calculateTime(
          seconds,
          this.customRunDistanceInMeters,
          "m",
          ratio
        );
      },
      ...
      calculateRunTimesByDistanceAndTime() {
        ...
        const seconds = minutesInput * 60 + secondsInput;
        const ratio = 1000;
        const pacePerKmInSeconds =
          (ratio / parseInt(this.runDistanceInputInMeters)) * seconds;
      ...
      calculateRunTimesByVelocity() {
        const velocityInput = parseFloat(this.runVelocityInputInKmh);
        const pacePerKmInSeconds = Math.round(3600 / velocityInput);
        const ratio = 1000;
        this.runTimesShortTrack.forEach((runTime) => {
          runTime.time = calculateTime(
            pacePerKmInSeconds,
...
function calculateTime(secondsInput, distance, distanceUnit, paceRatio) {
  const resultInSeconds =
    (secondsInput / paceRatio) * distanceInMeters(distance, distanceUnit);
...
function formatTime(hours, minutes, seconds) {
  return `${formatTimeElement(hours)}:${formatTimeElement(
    minutes
  )}:${formatTimeElement(seconds)}`;
}
